...
class MagicLinkFlow(AuthMixin, PasswordMixin):
    ...
    def send_magic_link(self, email_address: str):
        ...
        try:
            user = User.read(self.db_session, email_address=self.standardized_email(email_address))
    ...
    def send_email(self, email_address:str, slug:str):
        ...
        mailserver = smtplib.SMTP(
            settings.stmp_email_host,
            settings.smtp_email_port)
        mailserver.ehlo()
        mailserver.starttls()
        mailserver.ehlo()
        try:
    ...
    def _validate_email_settings(cls) -> bool:
        ...
        for key in ["smtp_email_host", "smtp_email_port", "smtp_email_user", "smtp_email_password"]:
