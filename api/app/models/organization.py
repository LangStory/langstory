...
class Organization(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(), nullable=False, doc="The name of the organization"
    )
    email_domain: Mapped[Optional[str]] = mapped_column(
        String(),
        doc="If set, all users with this email domain will be granted access to this organization",
    )
    avatar_url: Mapped[Optional[HttpUrl]] = mapped_column(
        String(), doc="The URL of the organization's avatar"
    )

    # relationships
    projects: Mapped[List["Project"]] = relationship(
        "Project", back_populates="organization", lazy="dynamic"
    )
    personas: Mapped[List["Persona"]] = relationship(
        "Persona", back_populates="organization", lazy="dynamic"
    )

    users: Mapped[List["User"]] = relationship(
        "User", secondary="organizations_users", lazy="selectin"
    )
